...
import { CLOUDINARY_IMAGE_URL } from "../../config/api";
import { inputsWithoutNominee, nomineeData } from "../../data/data";

export const A4Paper = ({ data, reference, image }) => {
  return (
    <div ref={reference} className="a-4 p-5 pt-10 relative">
      ...
      <div className="flex flex-wrap mt-2">
        ...
        {inputsWithoutNominee
          ?.slice(0, inputsWithoutNominee.length - 2)
          .map((v, i) => {
        ...
        {data.gender === "عورت" &&
          inputsWithoutNominee
            ?.slice(
              inputsWithoutNominee.length - 2,
              inputsWithoutNominee.length
            )
            .map((v, i) => {
              return <KeyValue key={i} data={data} v={v} />;
            })}
