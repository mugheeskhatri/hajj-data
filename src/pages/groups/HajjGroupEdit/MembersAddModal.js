...
function MembersAddModal({
  ...
  const selectMember = (user) => {
    ...
    if (onlyKeys.includes(user.NIC)) {
      ...
      setData((prev) =>
        user.NIC === data.leaderId
          ? { ...prev, members: membersToSet, leaderId: "" }
          : { ...prev, members: membersToSet }
      );
...
const MembersAddModalList = ({ user, selectMember, selectedMembers, data }) => {
  ...
  return (
    <TableRow
      ...
      style={
        user?.groupDetails && user?.groupDetails.groupId !== data.groupId
          ? { opacity: 0.8 }
          : {}
      }
    >
      ...
      <TableCell>
        {user?.groupDetails ? <Badge>{groupName}</Badge> : ""}
      </TableCell>
    </TableRow>
  );
};
