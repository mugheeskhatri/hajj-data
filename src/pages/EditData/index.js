...
function EditData() {
  ...
  const getData = async () => {
    setLoading(true);
    const response = await fetchDataFromFirebase(params.id);
    await setData({
      ...response,
      NIC: decodedField(response.NIC),
      nomineeNic: decodedField(response.nomineeNic),
      passport:
        response.passport.slice(0, 2) +
        decodedField(response.passport.slice(2, response.passport.length)),
    });
    setImage(
      response.image ? CLOUDINARY_IMAGE_URL + response.image : DUMMY_IMAGE
    );
    setLoading(false);
  };
  ...
  return (
    <div>
        ...
        <div>
          <div className="flex">
            <div className="w-7/12">
              <PageTitle>فارم ایڈٹ کریں </PageTitle>
            </div>
          ...
          <div>
            {!previewCard ? (
              <div className="flex flex-wrap">
                ...
                {data.gender === "عورت" &&
                  inputs
                    .slice(inputs.length - 6, inputs.length - 4)
                    .map((v, i) => {
                      return (
                        <div
                          key={i}
                          className={`w-full md:w-6/12 md:px-3 mt-5`}
                        >
                          {v.type !== "dropdown" ? (
                            <Label>
                              <span>{v.name}</span>
            ...
            <div className="my-5 px-3 w-full flex gap-2">
              <Button
                onClick={async () => {
                  const response = await handleSubmitForm(data, true);
                  if (response === "done") {
                    formDataSet();
      ...
      {previewCard && (
        <A4Paper data={data} reference={componentRef} image={image} />
      )}
    </div>
  );
}
